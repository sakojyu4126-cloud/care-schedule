...
import {
  formatTimeHHMM,
  parseTimeToMinutes,
  extractDailyActivities,
  getWingFromRoom,
  getShortenedServiceCode,
  normalizeHelperName,
  resolveHelperRoutesForDate,
  isInvalidHelperName
...
function HelperInstructionSection({
  ...
  return (
    <div className="mt-2.5 pt-2.5 border-t border-slate-200/80">
      <div className="font-bold text-slate-600 mb-1.5 flex items-center justify-between select-none text-[11px]">
        <div className="flex items-center gap-1">
          <AlertCircle className="w-3.5 h-3.5 text-indigo-600" />
          <span className="font-extrabold text-indigo-900">個別の申送り・連絡事項</span>
        </div>
        {onUpdateActivities && !isEditing && (
          ...
          >
            {activity.helperInstruction ? "編集" : "+ 記入する"}
          </button>
        )}
      </div>
      ...
      {isEditing ? (
        <div className="space-y-2 bg-white p-2.5 rounded-lg border border-indigo-200 shadow-2xs">
          <textarea
            ...
            placeholder="この時間・担当者への指示・連絡・報告事項を入力..."
            rows={3}
        ...
        <div className="bg-amber-50/90 border border-amber-200 p-2.5 rounded-lg text-amber-950 font-bold text-xs whitespace-pre-wrap">
          {activity.helperInstruction}
        </div>
      ) : (
        <div className="text-[11px] text-slate-400 italic bg-slate-100/60 p-1.5 rounded-lg border border-dashed border-slate-200 text-center select-none">
          個別申し送りはありません
        </div>
      )}
    </div>
...
export default function MobileHelperView({
  ...
  // 2. Filter active helper routes to match visible columns
  ...
  // Pure daily activities: Use PC activities directly without mixing unsaved/intermediate temporary reports
  const dateActivities = React.useMemo(() => {
    const directActs = activities.filter(a => a.date === selectedDate);
    if (directActs.length > 0) {
      return directActs;
    }
    // Auto-extract from weekly master if not yet initialized
    return extractDailyActivities(selectedDate, clients, settings);
  }, [activities, selectedDate, clients, settings]);

  // Filter activities based on selected helper name, selected date & search query
  const filteredActivities = React.useMemo(() => {
    const raw = dateActivities.filter((act) => {
  ...
  }, [dateActivities, selectedHelper, searchQuery, resolvedHelperRoutes]);
        ...
        <div className="flex items-center justify-between bg-slate-50 rounded-xl p-1.5 border border-slate-200/80 relative">
          ...
          {showCalendarModal && (
            <>
              ...
              <div 
                className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 bg-white rounded-2xl shadow-xl w-[280px] overflow-hidden border border-slate-200 animate-in fade-in slide-in-from-top-2 duration-150"
